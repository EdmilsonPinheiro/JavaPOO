...
public class Jogador2 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < jogadores.length - 1; i++) {
            for (int j = 0; j < jogadores.length - i - 1; j++) {
                // Comparação pelo atributo cidadeNascimento
                int comparacaoCidade = jogadores[j].getCidadeNascimento().compareTo(jogadores[j + 1].getCidadeNascimento());

                if (comparacaoCidade > 0 || (comparacaoCidade == 0 && jogadores[j].getNome().compareTo(jogadores[j + 1].getNome()) > 0)) {
                    // Swap dos jogadores
