...
class TabelaHash {
    ...
    private final int TAMANHO_TABELA = 79;
    ...
    private int funcaoHash(int altura, int k) {
        return ((altura % TAMANHO_TABELA) + k * (altura % 23)) % TAMANHO_TABELA;
    }
  
    void inserir(Jogador jogador) {
        int k = 0;
        while (true) {
            int pos = funcaoHash(jogador.getAltura(), k);
            if (tabela.get(pos).isEmpty()) {
...
public class hashAberto {
  ...
  public static void main(String[] args) throws IOException {
	  hashAberto aplicacao = new hashAberto();
  ...
  private static void inserirJogadores(Scanner scanner, TabelaHash tabelaHash, hashAberto aplicacao) {
      String entrada;
      do {
  ...
  private static void gerarArquivoLog(long tempo, int numComparacoes) throws IOException {
      try (FileWriter writer = new FileWriter("793938_rehashing.txt")) {
